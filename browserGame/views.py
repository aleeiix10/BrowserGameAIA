...
def play_action(request):
    if request.user.is_authenticated:
        ...
        json_msg= profileAjaxActions(request)
        json_str = json_msg.content.decode('utf-8')
        ...
        print(msg)
        return render(request, 'browserGame/play_action.html', {'form': form, "msg":msg})
            ...
            #Si acierta el ataque
                ...
                if action_used.category == "A":
                    ...
                    if (user_attacked.current_life - action_used.points) <= 0:
                        ...
                        enemyDie= True
                        nameUserAttacked= user_attacked.username
...
class UserAttackedForm(forms.Form):
    jugador_a_atacar = forms.ModelChoiceField(queryset=User.objects.all(), required=True)

class ActionForm(forms.Form):
    button_choice = forms.ModelChoiceField(queryset=Action.objects.all(),required=True)
    ...
    def __init__(self, user,*args, **kwargs):
        ...
        options = [(None, "Selecciona una acció")]
